...
Value sendmessagefrom(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 2)
        throw runtime_error(
            ...
            "\nSent an message using funds from an account to a ifp address.\n" +
            HelpRequiringPassphrase() + "\n"
                                        ...
                                        "\nExamples:\n"
                                        "\nSend \"hello ifp\" from the default account to the address.\n" +
            HelpExampleCli("sendmessagefrom", "\"\" \"XwnLY9Tf7Zsef8gMGL2fhWA9ZmMjt4KPwg\" \"hello ifp\"") +
            "\nSend \"hello ifp\" from the tabby account to the given address.\n" + HelpExampleCli("sendmessagefrom", "\"tabby\" \"hello ifp\"") +
            "\nAs a json rpc call\n" + HelpExampleRpc("sendmessagefrom", "\"tabby\", \"hello ifp\""));
    ...
    if (MESSAGE_FEE > nBalance)
        throw JSONRPCError(RPC_WALLET_INSUFFICIENT_FUNDS, "Account has insufficient funds (0.001 IFP required)");
...
Value sendmessage(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "\"transactionid\"  (string) The transaction id.\n"
            "\nExamples:\n" +
            HelpExampleCli("sendmessage", "\"hello ifp\""));
