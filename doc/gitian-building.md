...
*Setup instructions for a gitian build of IFP using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the IFP
...
Clone the git repositories for ifp and gitian and then checkout the ifp version that you want to build.
...
Follow the instructions in [doc/release-process.md](release-process.md) in the ifp repository
...
To build IFP (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the ifp repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/ifp/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/IFPCRYPTO/IFP
...
./bin/gbuild --commit ifp=${COMMIT} --url ifp=${URL} ../ifp/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit ifp=${COMMIT} --url ifp=${URL} ../ifp/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit ifp=${COMMIT} --url ifp=${URL} ../ifp/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/ifp-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/ifp-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/ifp-build.assert
```
...
[ifp/gitian.sigs](https://github.com/ifp-crypto/gitian.sigs/) repository, or if that's not possible to create a pull
request.
